...
SELECT
    ...
    sa_ent.subscription_agreement_status_label AS "Status",
...
WHERE
 ...
 -- Start with parameter filters
    AND
	(( sa_ent.subscription_agreement_status_label = (SELECT agreement_status FROM parameters)) OR
		((SELECT agreement_status FROM parameters) = ''))
	AND
    ((pci_list.pci_access_start::date < (SELECT pci_end_date FROM parameters) AND
	    (pci_list.pci_access_end::date >= (SELECT pci_start_date FROM parameters) OR pci_list.pci_access_end IS NULL))
	    OR
		(((SELECT pci_start_date FROM parameters) IS NULL)
			OR ((SELECT pci_end_date FROM parameters) IS NULL)))
	AND
    ((sa_ent.entitlement_active_from::date < (SELECT ent_end_date FROM parameters) AND
	    (sa_ent.entitlement_active_to::date >= (SELECT ent_start_date FROM parameters) OR sa_ent.entitlement_active_to IS NULL))
	    OR
...
GROUP BY
    ...
    sa_ent.subscription_agreement_status_label,
